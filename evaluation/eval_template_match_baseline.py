...
def main():
    ...
    # 先对所有 usable_examples 计算 Format + ROUGE
    for ex in usable_examples:
        ...
        try:
            score = rouge.get_scores(cand_seg, ref_seg)
            rouge_scores.append(score[0]["rouge-1"]["f"])
        except Exception:
            continue

        baseline_outputs.append(cand)

    # 然后单独构造一批样本做 BERTScore（一次性算）
